...
def create_database(loader_path,database_directory):
#函数：创建数据库
#输入：loader_path,列表，每一项为文件名；database_directory，保存路径
    # 1. 加载 PDF
    docs = []#PDF对象
    ...
    # 4. 创建向量数据库
    vectordb_chinese = Chroma.from_documents(#创建向量数据库
        documents=splits,#文档采用分割后的文档版本
        embedding=embedding,#向量化对象设置
        persist_directory=database_directory#保存路径
    )
    print("创建成功，数据库规模：",vectordb_chinese._collection.count())
    return
...
if __name__ == '__main__':
    ...
    create_database(loader_path=loader_path,database_directory=database_directory)
